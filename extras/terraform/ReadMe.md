...
- In `extras/helm/schiff/templates/deployment.yaml` müssen die Pfade bei `spec.template.spec.volumes[].hostpath`
  angepasst werden
...
Bis der Endpoint für den Service "schiff" verfügbar ist, muss man ggf. ein
...
Name _schiff_ aufgerufen wird. Alternativ kann auch das Skript `port-forward.ps1`
(s.o.) aufgerufen werden.

```powershell
    kubectl port-forward svc/schiff 8443 -n acme
```
...
Statt Port-Forwarding kann man auch in `extras\helm\schiff\templates` in
